...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (temImagens && GEMINI_API_KEY) {
      const modelosGemini = ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-1.5-flash-latest"];
      let errosLogs = [];
    ...
    if (!temImagens && texts) {
      ...
      if (GEMINI_API_KEY) {
        const modelosGeminiText = ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-1.5-flash-latest"];
